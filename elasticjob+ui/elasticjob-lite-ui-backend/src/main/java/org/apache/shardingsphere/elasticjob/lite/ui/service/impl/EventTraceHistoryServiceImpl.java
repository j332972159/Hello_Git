...
public final class EventTraceHistoryServiceImpl implements EventTraceHistoryService {
    ...
    public Page<JobExecutionEvent> findJobExecutionEvents(final FindJobExecutionEventsRequest findJobExecutionEventsRequest) {
        ...
        Pageable pageable = getPageable(findJobExecutionEventsRequest, JobExecutionLog.class);
        Page<JobExecutionLog> page = jobExecutionLogRepository.findAll(specification, pageable);

        return new PageImpl<>(page.getContent().stream().map(JobExecutionLog::toJobExecutionEvent).collect(Collectors.toList()), pageable, page.getTotalElements());
    }
    ...
    public Page<JobStatusTraceEvent> findJobStatusTraceEvents(final FindJobStatusTraceEventsRequest findJobStatusTraceEventsRequest) {
        ...
        Pageable pageable = getPageable(findJobStatusTraceEventsRequest, JobStatusTraceLog.class);
        Page<JobStatusTraceLog> page = jobStatusTraceLogRepository.findAll(specification, pageable);
        return new PageImpl<>(page.getContent().stream().map(JobStatusTraceLog::toJobStatusTraceEvent).collect(Collectors.toList()), pageable, page.getTotalElements());
    }
